...
typedef struct OQS_KEX {
    OQS_RAND *rand;                         // PRNG
    char *method_name;                      // Specifies the name of the key exchange method.
    uint16_t estimated_classical_security;  // Classical security in terms of the number of bits provided by the key exchange method.
    uint16_t estimated_quantum_security;    // Equivalent quantum security in terms of the number of bits provided by the key exchange method.
    uint8_t *seed;                          // An instance-specific seed, if any.
    size_t seed_len;                        // Size of instance-specific seed, if any.
    char *named_parameters;                 // Named parameters for this key exchange method instance, if any.
    void *params;                           // Opaque pointer for passing around instance-specific data.
    void *ctx;                              // Opaque pointer for passing around any computation context.
